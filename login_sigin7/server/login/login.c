...
int login(int sockfd,msg_t msg)
{
    ...
    {
        if(cli_info[i].cli_sockfd == -1 || cli_info[i].cli_sockfd == 0)
        {
    ...
    for(int i = 0 ; i < 3;i++)
    {
        printf("cli_info[%d].fd = %d\n",i,cli_info[i].cli_sockfd);
        printf("cli_info[%d].name = %s\n",i,cli_info[i].cli_name);
    }
    msg_t ret_msg;
