...
{
    char message[1024];
    memcpy(message,msg.val,sizeof(msg.val));
    ...
    else
    {
        for(int i = 0 ; i < MAX_CLI_SIZE;i++)
        {
            ...
            {
                sprintf(msg.val,"%s: %s",cli_info[i].cli_name,message);
                break;
            }
        }
        for(int i = 0 ; i < MAX_CLI_SIZE;i++)
...
int get_luckey_money(int sockfd,msg_t msg)
{
    int other_fd ;
    for(int i = 0 ; i  < MAX_CLI_SIZE;i++)
    {
        ...
        break;
        if(sockfd == cli_info[i].cli_sockfd)
            memcpy(myname,cli_info[i].cli_name,sizeof(cli_info[i].cli_name));
        if(!strcmp(cli_info[i].cli_name,msg.val))
            other_fd = cli_info[i].cli_sockfd;
    }
}
